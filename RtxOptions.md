...
|rtx.sharc.accumulationFrameNum|int|20|||Number of frames to accumulate radiance into the cache before resolving\.<br>Lower values respond faster to lighting changes; higher values are smoother\.|
|rtx.sharc.capacityLog2|int|22|||Log2 of the hash\-map capacity \(default 22 => 4,194,304 entries => 176 MiB\)\.<br>Changing this requires restarting the application\.|
|rtx.sharc.debugMode|int|0|||SHARC debug visualisation mode\.<br>0: Off, 1: HashGridColor, 2: Occupancy, 3: HashCollisions, 4: BitsOccupancy, 5: CachedRadiance\.|
|rtx.sharc.downscaleFactor|int|5|||Screen\-space downsample factor for cache queries\.<br>Higher = fewer cache lookups per frame, less overhead, but coarser coverage\.|
|rtx.sharc.enable|bool|False|||Enables SHARC \(Spatially Hashed Radiance Cache\) for indirect illumination\.<br>When enabled, SHARC replaces ReSTIR GI and NRC as the indirect path\.<br>Requires RayQuery or RayQueryRayGen raytrace mode\.<br>Allocates ~176 MiB of GPU memory at capacity 2^22\.|
|rtx.sharc.enableAntiFireflyFilter|bool|True|||Enables anti\-firefly clamping on accumulated radiance in the cache\.<br>Reduces rare bright pixels caused by outlier samples\.|
|rtx.sharc.enableMaterialDemodulation|bool|True|||Enables SHARC material demodulation \(divide by albedo before storing, re\-apply on lookup\)\.<br>Keeps stored radiance albedo\-agnostic for better cache reuse\.|
|rtx.sharc.enableQuery|bool|True|||Enables cache queries during path termination\.<br>Disable to measure the overhead of full path tracing without cache lookups\.|
|rtx.sharc.enableUpdate|bool|True|||Enables the cache update pass \(writing new radiance samples\)\.<br>Disable to benchmark query\-only overhead or freeze the cache\.|
|rtx.sharc.roughnessThreshold|float|0.4|||Surfaces with roughness below this value bypass SHARC and use direct path tracing\.<br>Prevents blurry cache lookups on near\-specular surfaces\.|
|rtx.sharc.sceneScale|float|50|||World\-space scale for the hash\-grid cell size\.<br>Increase for large open\-world scenes; decrease for small indoor scenes\.<br>Affects how finely the world is partitioned into radiance cells\.|
|rtx.sharc.staleFrameNumMax|int|60|||Maximum age \(in frames\) before a cache entry is considered stale and evicted\.|
|rtx.sharc.updateProbability|float|1|||Probability \[0,1\] that a given pixel contributes an update sample to the cache\.<br>Reducing below 1 lowers update overhead at the cost of slower cache warm\-up\.|
